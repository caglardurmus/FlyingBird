...
export const addPipesAtLocation = (x, world, entities) => {
    ...
    let pipeTopHeight = (pipeTopWidth / 51) * 23; // original image is 205x95
...
const Physics = (entities, {touches, time, dispatch}) => {
    ...
    touches.filter(t => t.type === 'press').forEach(t => {
        if (!hadTouches) {
            if (world.gravity.y === 0.0) {
                // first press really
                dispatch({type: 'start-end'});
                delete (entities['startImage']);
                world.gravity.y = 1.2;
    ...
    Object.keys(entities).forEach(key => {
        ...
        } else if (key.indexOf('floor') === 0 || key.indexOf('background') === 0) {
